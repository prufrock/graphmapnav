...
fun main(args: Array<String>) = application {
    ...
    program {
        ...
        extend {
            ...
            // try out batch rendering with a little hack
            val rectangles: MutableList<Graphics> = mutableListOf()
            val lines: MutableList<LineSegment> = mutableListOf()
            val textTexts: MutableList<String> = mutableListOf()
            val textLocations: MutableList<Vector2> = mutableListOf()
            ...
            game.world.graphics.render { v ->
                ...
                when(val model = graphics.model) {
                    ...
                    is Label -> {
                        val (text, location) = model.text(graphics, game, width = width + 0.0, height = height + 0.0, fontBook = fontBook, zoom = zoom)
                        textTexts.add(text)
                        textLocations.add(location)
                    }
                    is Square -> {
                        rectangles.add(graphics)
                    }
                    is Line -> {
                        lines.add(model.lineSegment(graphics, game, width = width + 0.0, height = height + 0.0, fontBook = fontBook, zoom = zoom))
                    }
                    else -> {
            ...
            drawer.rectangles {
                rectangles.forEach {graphics: Graphics ->
                    graphics.model.draw(this, graphics, game, width = width + 0.0, height = height + 0.0, fontBook = fontBook, zoom = zoom)
                }
            }
            val selectedFont = when(zoom) {
                in 0.0..0.8 -> fontBook.small
                in 0.8..1.3 -> fontBook.medium
                in 1.3..2.0 -> fontBook.large
                else -> fontBook.large}

            drawer.fontMap = selectedFont
            drawer.texts(textTexts, textLocations)
